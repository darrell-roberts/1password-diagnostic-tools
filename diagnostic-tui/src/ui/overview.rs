...
use crate::app::{InputMode, OverviewState, Tab};
...
use diagnostic_parser::model::DiagnosticReport;
...
impl StatefulWidget for OverviewWidget<'_> {
    ...
    fn render(self, area: Rect, buf: &mut Buffer, state: &mut OverviewState) {
        ...
        let total_lines_str = self.total_log_lines.to_string();
        lines.push(kv_line("Total Lines", &total_lines_str));
        let parsed_str = self.all_entries.len().to_string();
